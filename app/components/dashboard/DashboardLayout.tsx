...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-950">
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-72 border-r border-slate-200 bg-white transition lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="flex h-full overflow-auto flex-col sidebar-scrollbar">
          <div className="flex h-16 items-center justify-between border-b border-slate-200 px-5">
            <div>
              <h1 className="text-base font-semibold">LMS Dashboard</h1>
              <p className="text-xs capitalize text-slate-500">
                {effectiveRole?.replace("_", " ") || "admin"}
              </p>
            </div>
            <button
              type="button"
